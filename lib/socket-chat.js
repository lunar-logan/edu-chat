    ...
    io.on('connection', function (socket) {
        ...
        function pushMessage(message) {
            if (message.type === 'user') {
                ...
                models.GroupMember.findAll({
                    where: {groupUid: message.toUser}
                }).then(function (members) {
                    if (members) {
                        members.forEach(function (member) {
                            if (member.userUid in onlineUsers && member.userUid !== message.fromUser) {
                                // Send message to all the members except the one who sent it
                                onlineUsers[member.userUid].emit('private message', {
                                    id: message.id,
                                    fromUser: message.toUser,
                                    toUser: member.userUid,
                                ...
                                console.log(colors.yellow('User: ' + member.userUid + ' is not online at the moment'));
                            }
                        });
                    } else {
                        console.log(colors.red('Group: ' + message.toUser + ' seems to have no members'));
                    }
                });
            }
        }
        ...
        socket.on('private message', function (message) {
            ...
            if (message.type === 'user') {
                ...
                console.log(colors.red("Message of unknown type: " + message.type));
            }
        });
        ...
        socket.on('create group', function (msg) {
            ...
            console.log(colors.yellow("Requested to create a group: " + JSON.stringify(msg)));
            if (socket.handshake.session.authToken) { // authorized to create a group
                ...
                models.Group.create({
                    ...
                    unique(msg.members).forEach(function (member) {
